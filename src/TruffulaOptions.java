...
public class TruffulaOptions {
  ...
  public TruffulaOptions(String[] args) throws IllegalArgumentException, FileNotFoundException {
    if (args.length < 1) throw new IllegalArgumentException("Missing arguments. Must provide at least a path.");
    ...
    for (int i = 0; i < args.length - 1; i++) {
      String flag = args[i].trim().toLowerCase();
      switch (flag) {
        case "-h":
          tempShowHidden = true;
          break;
    ...
    File possibleRoot = new File(path);
    if (!possibleRoot.exists()) throw new FileNotFoundException("Directory does not exist: " + path);
    if (!possibleRoot.isDirectory()) throw new FileNotFoundException("Path is not a directory: " + path);

    this.root = possibleRoot;
    this.showHidden = tempShowHidden;
    this.useColor = tempUseColor;
  }
